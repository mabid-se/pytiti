            ...
            <div class="live-preview">
              <div class="table-responsive table-card">
                <table class="table">
                  <thead>
                    <tr>
                      <td v-for="(head, index) in columns" :key="head">
                        {{ head.label }}
                      </td>
                    </tr>
                  </thead>
                ...
                <!--
                                ...
                                >
                                    <template v-slot:title>
                                        <h4 class="card-title mb-0 flex-grow-1">{{ items[1].text }}</h4>
                                    </template>
                                        ...
                                        <tr>
                                            <th scope="col" v-for="(item, index) in columns" :key="index">
                                                <div class="d-flex justify-content-between">
                                                    {{ item.label }}<i v-if="item.showOrderIcon" @click="ordering=item.orderKey" class="las la-sort la-lg my-auto pointer"></i>
                                                </div>
                                            </th>
                                        </tr>
                                    </template>
                                    <template v-slot:row="{ item }">
                                        <tr>
                                            ...
                                            <td>{{ item.fid_tramite.objetivo }}</td>
                                            <td>
                                                <span class="fw-normal fs-9" :class="stateClassAccount(item)">
                                                    <i :class="stateIconAccount(item) + ' fs-9 align-middle'"></i> {{ item.estado }}
                                                </span>
                                            </td>
                                            <td>
                                              <div class="list-grid-nav hstack gap-1">
                                                <a href="javascript:void(0);" class="btn btn-info btn-label right" id="dropdownMenuLink1" data-bs-toggle="dropdown" aria-expanded="false" data-v-3b9a351b="">
                                                    <div class="d-flex">
...
export default {
  ...
  data() {
    return {
      title: "Listado de Trámites en Curso",
      items: [
        {
          text: "Solicitud",
          href: "/",
  ...
  mounted() {
    fetch(
      `${process.env.VUE_APP_PROD_URL}api/v1/persona/${this.userType}/tramites/`,
      {
        headers: {
