                ...
                >
                  ...
                  <template v-slot:row="{ item }">
                    <tr>
                      ...
                      <!--td v-if="userIsAdmin">{{ accountType(item) }}</td-->
                      <td>
                        <div class="fw-bold text-muted" v-if="userIsAdmin">
                          {{ accountType(item) }}:
                        </div>
                        <div v-if="!!item.tramitador">
                          {{ userAccount(item) }}
                        </div>
                        <div v-else>
                          {{ userAccount(item) }}
                      ...
                      <td>
                        <div class="d-flex flex-wrap gap-2">
                          <AccionesLista
                            @send-action="selectPreRequest(item)"
                            :itemData="item"
                          ...
                          >
                            <template #verDetalle>
                              <router-link
                                @click="selectPreRequest(item)"
                                class="dropdown-item"
                                :to="{
                                  name: 'tramite-pre-solcitado-detalle',
                                  params: {
                                    requestId: item.id_pre_solicitud,
                                  },
                                }"
                              >
                                Ver Detalle
...
export default {
  ...
  computed: {
    loadProceduresUrl() {
      if (this.userIsAdmin) {
        ...
        return `empresa/${this.userType}/pre-solicitudes/`;
      }
      return `persona-natural/${this.userType}/pre-solicitudes/`;
    },
    unprivilegedColumns() {
  ...
  methods: {
    userAccount(item) {
      ...
      if (this.userIsCompany || this.userIsPerson) {
        return item.tramitador === null
          ? item.empresa.razonsocial
          : `${item.tramitador.fid_persona.nombres} ${item.tramitador.fid_persona.apellido_paterno}`;
      }
    },
    dateFormat(data) {
