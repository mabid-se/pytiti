                ...
                >
                  <template v-slot:title>
                    <h4 class="card-title mb-0 flex-grow-1">
                      Lista Solicitud Registro Tramitadores
                    </h4>
                  </template>
                  <template v-slot:filters>
                    <label class="form-label col-auto fw-bold"
                      >Filtrar solicitud registro tramitadores por:</label
                    >
                    <div class="col-md-3">
                      <input
                        type="text"
                      ...
                      <td>
                        <div class="d-flex flex-wrap gap-2">
                          ...
                          >
                            <template #verDetalle>
                              <!-- <router-link
                                @click="selectProcessor(item)"
                                class="dropdown-item"
                                :to="
                                  userIsAdmin
                                    ? `/tramitador/${item.fid_persona.idpersona}`
                                    : `/tramitador/${item.idtramitador}`
                                "
                              >
                                Ver Detalle
                              </router-link> -->
...
export default {
  ...
  data() {
    return {
      title: "Listado de Solicitudes de Registro Tramitadores",
      items: [
        {
          text: "Solicitud",
          href: "/",
  ...
  methods: {
    ...
    loadProcessorAccounts() {
      if (userIsAdmin()) {
        return `cuenta/tramitadores/solicitudes/${this.orderString}`;
      } else {
        return `${this.userType}/tramitadores/${this.orderString}`;
      }
    },
    dateFormat(data) {
