<script>
    ...
    import {getRolesSiaService, getPermissionsSiaService} from "@/services/permisos";
    ...
    export default {
      ...
      data() {
        return {
          ...
          usersiadata: {},
          checkeditems: {},
          services_internet: false,
          services_correo_institucional: false,
          ...
          modalData: {
                ...
                buttonText: "Aceptar",
                redirectUrl: "/accesosia/solicitudessia"
          },
        };
      },
      name: "Widgets",
      ...
      methods: {
        ...
        reviewed_aprobar: function (event) {
          ...
          let _services = {
            "correo_institucional": this.services_correo_institucional,
            "internet": this.services_internet
          }
          let solicitudes_adicionales = {
            "solicitudes_adicionales": this.solicitudes_adicionales,
          }
          let data = {"items_to_validate": result, "services": _services, "solicitudes_adicionales": solicitudes_adicionales };// "is_vo_bo": this.vo_bo};
        ...
        approve () {
            ...
            attachedAndApproveService(this.file[0], this.usersiadata.id, this.uid).then(
                (res) => {
                    if (res.result['message'] == "successfully"){
                        this.modalData["message"] = "El usuario "+ this.usersiadata.user.first_name + " se valido correctamente."
        ...
        aditional_data (services, aditionals) {
            if(services){
                if(services.internet) {
                    this.services_internet = services.internet;
                }
                if(services.correo_institucional) {
                    this.services_correo_institucional = services.correo_institucional;
                }
            }
            if(aditionals){
                if(aditionals.solicitudes_adicionales) {
                    this.solicitudes_adicionales = aditionals;
                }
            }
        }
      },
      mounted () {
        if (this.$store.state.authfack.user.profile){
            let admin_groups =  Object.values(this.$store.state.authfack.user.profile.groups);
            if(admin_groups.includes('interno-sia-sistemas-departamental') || admin_groups.includes('interno-sia-sistemas-nacional')) {
        ...
        if (this.usersiadata) {
            this.aditional_data(this.usersiadata['servicios'], this.usersiadata['solicitudes_adicionales'])
        }
        if(!this.usersiadata) {
            retrieveUserData(this.$route.params['uid'])
                .then(
                    (res) => {
                      const item = JSON.parse(JSON.stringify(res));
                      this.usersiadata = item.user_request;
                      console.log(this.usersiadata);
                      this.aditional_data(this.usersiadata['servicios'], this.usersiadata['solicitudes_adicionales'])
                    },
                    err => {
                        console.error(err);
        ...
        if (this.uid){
            ...
            retrieveUserTramitesSiaService(this.uid)
                .then(
                    (res) => {
                        const items = JSON.parse(JSON.stringify(res.results))
                        this.tramites = items;
        ...
        getPermissionsSiaService().then(
                (res) => {
                    const items = JSON.parse(JSON.stringify(res.results))
                    let admins_roles = _.filter(items, function(o) { return o.permission_type == 'administration'; });
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="col-xxl-12">
                        <div class="">
                            <button style="visibility: hidden" data-bs-toggle="modal" data-bs-target=".bs-example-modal-center" id="close"></button>
                            <div class="modal fade bs-example-modal-center" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="mt-4">
                                        ...
                                        <p class="text-muted mb-4">Se procedera a validar por el encargado de sistemas</p>
                                        <div class="hstack gap-2 justify-content-center">
                                            <button type="button" class="btn btn-light" data-bs-dismiss="modal" @click="backPage()">Cerrar</button>
                                        </div>
                                    </div>
                                    </div>
                                </div>
                                                            ...
                                                            <div class="live-preview">
                                                                <div>
                                                                    ...
                                                                    <div v-if="sia_is_system">
                                                                        <h5 class="text-muted text-uppercase fs-13">PERMISOS DE AMINISTRACIÓN:</h5>
                                                                        <div class="row g-3 mb-3 ml-sm">
                                                                            <div class="form-check mb-1" v-for="(data, index) of groups" :key="index">
                                                                                <input class="form-check-input" type="checkbox" :name="data.name"
                                                                                    @change="addPermision(data)"
                                                                                    v-model="data.checked" :value="data.value" :id="index" :disabled="attached_file">
                                                                                <label :for="data.name" class="form-check-label">
                                                                                    {{data.name}}
                                                                                </label>
                                                                            </div>
                                                                            <div class="form-check form-check-dark">
                                                                                <input class="form-check-input" type="checkbox" 
                                                                                    id="formCheck12" @click="selectAll" v-model="all_selected_group"
                                                                    ...
                                                                    <div class="row g-3 mb-3 ml-sm">
                                                                        <div class="form-check mb-2 col-lg-4">
                                                                            <input class="form-check-input" type="checkbox" v-model="services_internet" :disabled="attached_file">
                                                                            <label class="form-check-label"> INTERNET </label>
                                                                        </div>
                                                                        <div class="form-check mb-2 col-lg-4">
                                                                            <input class="form-check-input" type="checkbox" v-model="services_correo_institucional" :disabled="attached_file">
                                                                            <label class="form-check-label" > CORREO INSTITUCIONAL</label>
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    <div class="row g-3 mb-3" v-if="!attached_file" >
                                                                                ...
                                                                                <div class="card-body-full">
                                                                                    <div class="live-preview">
                                                                                        <b-accordion>
                                                                                                ...
                                                                                                <div class="card-full">
                                                                                                    ...
                                                                                                    <div class="table-responsive table-card">
                                                                                                        ...
                                                                                                        <thead class="table-light text-muted">
                                                                                                            ...
                                                                                                            <th scope="col" width="180px">Rol</th>
                                                                                                            </tr>
                                                                                                        </thead>
                                                                                                        <!--end thead-->
                                                                                                        <tbody class="list form-check-all">
                                                                                                            ...
                                                                                                            <td class="id">
                                                                                                                {{ data.tramite.fid_servicio }}
                                                                                                            </td>
                                                                                                            <td>{{ data.tramite.title }}</td>
                                                                                                            <td>
                                                                                                                <div class="form-check mb-2">
                                                                                                                <input class="form-check-input mr-1" type="checkbox" 
                                                                                                                    :value="data.enabled" v-model="data.enabled" :checked="data.enabled"
                                                                                                                    @change="updateCheckTramit(data)"
                                                                                                                >
                                                                                                                <label class="form-check-label" for="formCheck1"></label>
                                                                                                                </div>
                                                                                                            </td>
                                                                                                            <td>
                                                                                                                <select v-model="data.rol" class="form-select" aria-label="Default select example" @change="onChangeRol(data)">
                                                                                                                    <option :value="null" selected disabled>Asignar un rol</option>
                                                                                                                    <option v-for="(option, index) in options_roles" :key="index" :value="option.id">
                                                                                                                        {{ option.name }}
                                                                                                                    </option>
                                                                                                                </select>
                                                                                                            </td>
                                                                                            ...
                                                                                            <b-accordion-item v-for="(data, title) of grouped_system" :title="title" :key="title">
                                                                                                <div class="form-check mb-2 col-lg-6" v-for="(d, i) of data" :key="i">
                                                                                                    <input class="form-check-input" type="checkbox" :value="d.id" v-model="d.checked" :checked="d.checked" @input="updateCheckSystem">
                                                                        ...
                                                                        <div class="text-center mt-3">
                                                                            <button v-if="sia_is_system" type="button" class="btn btn-success waves-effect waves-light" @click="reviewed_aprobar">Guardar</button>
                                                                            <button v-else type="button" class="btn btn-success waves-effect waves-light" @click="reviewed_aprobar">Guardar y enviar a Sistemas</button>
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    <div class="row g-3 mb-3" v-if="attached_file" >
                                                                        ...
                                                                        <div class="cardbold">
                                                                            <div class="card-header align-items-center d-flex">
                                                                                <h5 class="card-title mb-0 flex-grow-1">Documentos de aprobación</h5>
                                                                            </div>
                                                                            <!-- end card header -->
                                                                            <div class="card-body">
                                                                                <div class="live-preview">
                                                                                    <div class="row gy-4">
                                                                                        <div class="col-sm-12 col-lg-12 col-xxl-12">
                                                                                            <div>
                                                                                                <b-button @click="print" variant="primary" class="btn btn-primary waves-effect waves-light mr-1 mb-4">Imprimir formulario de respaldo</b-button>
                                                                                            </div>
                                                                                            <div>
                                                                                                <label for="formFile" class="form-label">Adjuntar documento firmado</label>
                                                                                                <input class="form-control w-50" type="file" id="formFile" @change="onFileChange">
                                                                                            </div>
                                                                                        </div>
                                                                                        <!--end col-->
                                                                                    </div>
                                                                        ...
                                                                        <div class="col-12">
                                                                            <div class="text-center mt-3">
                                                                                <div class="card-body-">
                                                                                    <div class="live-preview-">
                                                                                        <!-- Base Buttons -->
                                                                                        <b-button variant="success" class="waves-effect waves-light mr-1" @click="approve">Guardar</b-button>
                                                                                    </div>
                                                                                </div>
                                                                                <!-- end card-body -->
                                                                            </div>
