        ...
        <div class="card">
          ...
          >
            <h6 class="card-title mb-0 flex-grow-1">
              <i class="bx bx-bookmarks fs-6"></i> Etapa:
              <b v-if="userIsAdmin">Evaluación documental</b>
              <span v-if="!userIsAdmin">{{ step.name }}</span>
            </h6>
          </div>
        </div>
        <!-- end card header -->
...
import { getAlmacenesAdmin } from "@/services/almacenes";

export default {
  name: "TramitePlantillaAdmin",
  ...
  methods: {
    // async load
    loadTemplate() {
      getProcedureTemplateForAdmin(this.$route.params.slug)
        .then((res) => {
          this.template = res.fid_tramite;
          this.steps = this.template.plantilla_tramite.steps;
          this.steps.map((step) => {
            this.components = step.components
              .filter((c) => c.name != "Clasificación de la Empresa")
              .map((c) => ({
                ...c,
                forms: c.forms.filter(
                  (f) =>
                    ![
                      "Documentación del representante legal",
                      "Documentación del técnico responsable",
                    ].includes(f.name)
                ),
              }));
            this.title = "TRAMITE: " + this.template.titulo;
          });
          getAlmacenesAdmin(res.fid_empresa.idempresa).then((res) => {
    ...
    createResourceTemplate(entityUrl, formData) {
      postStartedTemplate(entityUrl, formData).then(
        (res) => {
          console.log(res);
        },
        (err) => {
