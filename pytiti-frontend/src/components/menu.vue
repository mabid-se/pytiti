  ...
  <div class="container-fluid">
        ...
        <li class="nav-item">
          ...
          <div class="collapse menu-dropdown" id="sidebarAdvanceUI">
            <ul class="nav nav-sm flex-column">
              ...
              <li class="nav-item">
                <router-link
                  to="/advance-ui/nestable"
                  class="nav-link"
                  data-key="t-nestable-list"
                  >{{ $t("t-nestable-list") }}</router-link
                >
              </li>
              <li class="nav-item">
                <router-link
    ...
    <template v-else>
      <ul class="navbar-nav h-100" id="navbar-nav">
        <template v-for="(m, index) in menu" :key="`menu.${index}`">
          <li class="menu-title">
            <span> {{ m.heading }}</span>
          </li>
          <!-- end Dashboard Menu -->
          <template
            v-for="(ch1, indexL1) in m.children"
            :key="`children.${indexL1}`"
          >
            <!-- nested -->
            <li class="nav-item" v-if="ch1.nested">
              <a
                class="nav-link menu-link"
                :href="`#${ch1.to}`"
                data-bs-toggle="collapse"
                role="button"
                aria-expanded="true"
                :aria-controls="`${ch1.to}`"
              >
                <i :class="ch1.icon"></i>
                <span> {{ ch1.text }}</span>
              </a>
              <div class="collapse menu-dropdown" :id="ch1.to">
                <ul class="nav nav-sm flex-column">
                  <li
                    class="nav-item"
                    v-for="(ch2, indexL2) in ch1.children"
                    :key="`children.${indexL2}`"
                  >
                    <router-link
                      class="nav-link"
                      :to="ch2.to.startsWith('/') ? ch2.to : { name: ch2.to }"
                    >
                      {{ ch2.text }}
                    </router-link>
                  </li>
            ...
            <li class="nav-item" v-else>
              <router-link
                class="nav-link menu-link"
                :to="ch1.to.startsWith('/') ? ch1.to : { name: ch1.to }"
              >
                <i :class="ch1.icon"></i>
                <span>{{ ch1.text }}</span>
              </router-link>
