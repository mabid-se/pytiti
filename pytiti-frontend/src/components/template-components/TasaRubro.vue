<template>
                        ...
                        <div class="card-body">
                            <div class="live-preview">
                                <div class="row gy-4">
                                    ...
                                    <div v-if="!liquidationValidation.ok" class="col-sm-12 col-lg-12 col-xxl-12">
                                        <h4 class="card-title mb-2 flex-grow-1">Tasa y rubro</h4>
                                        <div class="mb-3">
                                            <select
                                            ...
                                            >
                                                <option
                                                    ...
                                                    :key="index"
                                                >
                                                    ({{ option.fid_servicio }}) {{ option.titulo }}
                                                </option>
                                            </select>
                                            <div id="basiInputAyuda" class="form-text">
                                    ...
                                    <div v-if="liquidationValidation.ok" class="col-sm-12 col-lg-12 col-xxl-12">
                                        <h4 class="card-title mb-2 flex-grow-1">El tramite cuenta con una liquidación vinculara, y no puede cambiar su tasa y rubro</h4>
                                        </div>
                                    <!--end col-->
                                </div>
                                <!--end row-->
                    ...
                    >
                        <thead>
                            <tr>
                                ...
                                <th v-if="!liquidationValidation.ok" scope="col">Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(servicio, index) of this.seleccionados" :key="index">
                                ...
                                <td v-if="!liquidationValidation.ok">
                                    <div
                                        class="btn-group"
                                        role="group"
    ...
    <div class="mb-3" v-if="!liquidationValidation.ok">
        <div class="row">
            <div class="btn_cen">
                <button type="button" class="btn btn-guardar btn-label right mb-1" @click="guardar">
...
import {getRubrosByTramite, getServicesByProcedure, registerCategoryNames,registerServices, registrarUnidadRubro} from "@/services/rubros"
...
export default {
    ...
    filters: {},
    methods: {
        async getTramites(unidad) {
            const {results} = await getProceduresList(unidad)
        ...
        async editar(id_rubro) {
            ...
            this.tramitesOpciones.push(
                this.tramitesOpcionesExtra.find(
                    (tramite) => tramite && tramite.id === editarData.tramiteSeleccionado
                )
            )
        ...
        async guardar() {
            const payload = this.seleccionados.map((item) => ({
                ...
                tramite_id: item.tramiteSeleccionado
            }))
            const payloadNombreCategoria =this.seleccionados.map(({rubro}) => rubro)
            await registerServices({
                solicitud_tramite_id: this.solicitud_tramite_id,
                servicios: payload
            })
            await registerCategoryNames({
                solicitud_tramite_id: this.solicitud_tramite_id,
                categorias: payloadNombreCategoria
            })
            await registrarUnidadRubro({
                solicitud_tramite_id: this.solicitud_tramite_id,
                idrubro: this.seleccionados.length ? this.seleccionados[0].idrubro:null,
                idunidad: this.seleccionados.length ? this.seleccionados[0].idunidad:null,
                fid_rubro: this.seleccionados.length ? this.seleccionados[0].rubroSeleccionado:null
            })
            NotificationHelper.success("Se guardo los servicios correctamente.");
        }
    },
    async beforeMount() {
        if (this.$route.params.id_solicitud_tramite) {
            this.solicitud_tramite_id = this.$route.params.id_solicitud_tramite
            const sucursal = await getInfo(this.solicitud_tramite_id)
            this.empresaInfo = sucursal
            await this.getTramites(sucursal.data.fid_tramite.unidad)
            const serviciosInfo = await getServicesByProcedure(
                this.$route.params.id_solicitud_tramite
            )
            if (serviciosInfo.length) {
                const filterTramitesIds = []
                this.seleccionados = serviciosInfo.map(({rubros_id, tramite_id}) => {
                    filterTramitesIds.push(rubros_id.codigoservicio)
                    return {
                        tramiteSeleccionado: tramite_id.id,
                        tramite: tramite_id.titulo,
                        rubroSeleccionado: rubros_id.id_rubros,
                        id_rubro: rubros_id.id_rubros,
                        idrubro: rubros_id.idrubro,
                        idunidad: rubros_id.idunidad,
                        rubro: rubros_id.nombrerubro,
                        codigoTasa: rubros_id.codigoservicio
                    }
                })
                this.tramitesOpciones = this.tramitesOpciones.filter(
                    (tramite) => !filterTramitesIds.includes(tramite.fid_servicio)
                )
            }
