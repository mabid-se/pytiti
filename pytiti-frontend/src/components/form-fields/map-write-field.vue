<template>
  <div v-if="field.type_field === 'map'">
    <label
      :for="field.id"
      :class="{ required: field.required, 'text-danger': errorVuelidate }"
      >{{ field.label }}</label
    >
          ...
          <ol-vector-layer :zIndex="2">
            <ol-source-vector>
              <ol-feature ref="positionFeature">
                <ol-geom-point
                  :coordinates="slotProps.position"
                ></ol-geom-point>
                <!--<ol-style>
      ...
      <div class="col-lg">
        <div class="collabse show" id="latLongCollapse">
          ...
          <div
            ...
            v-if="
              errorVuelidate && errorVuelidate[0] && errorVuelidate[0].$message
            "
          >
            {{ errorVuelidate[0].$message }}
          </div>
          <div class="error-validation" v-else-if="errorVuelidate">
            {{ errorVuelidate[0] }}
          </div>
        </div>
      </div>
...
export default {
  ...
  setup(props, { emit }) {
    let center = reactive({
      cnt: [-16.521280047628807, -68.12861682381676].reverse(),
    });
    ...
    const drawstart = (event) => {
      ...
      emit("update-coordinates", {
        latitud: String(latLong[1]),
        longitud: String(latLong[0]),
      });
    };
