...
import ForgotPasswordModal from "@/modules/public/views/forgot/forgot-password-modal";
import { setPasswordSiaService } from "@/services/auth";
...
export default {
  ...
  methods: {
    ...
    updateFormData(formData) {
      let correo = formData.data;
      setPasswordSiaService(correo).then(
          (res) => {
            if (res.message === "No encontrado."){
              this.$notify({
                title: "Validación",
            ...
            if (res.status === "solicitado"){
              this.$notify({
                title: "Validación",
                text: "Contraseña del usuario solicidata.",
                type: "info",
              });
            }
          },
...
<template>
  ...
  <ForgotPasswordModal
    @send-confirmation="updateFormData"
    :attrs="modalData"
  />
  <a
    id="show-modal"
