        ...
        <div class="card-body">
          <div class="live-preview">
            <div class="table-responsive table-card">
              <KTable
                v-if="isReady"
                :url="`admin/solicitud/vincular-tramitador/`"
                :current-limit="5"
                :query-params="queryParams"
              >
                <template v-slot:title>
                  <h4 class="card-title mb-0 flex-grow-1">
                    Lista Solicitud Vincular Tramitadores
                  </h4>
                </template>
                <template v-slot:filters>
                  <label class="form-label col-auto fw-bold"
                    >Filtrar solicitud vincular tramitadores por:</label
                  >
                  <div class="col-md-3">
                    <input
                      type="text"
                ...
                <template v-slot:row="{ item }">
                  <tr>
                    ...
                    <td>
                      {{ item.fid_tramitador.correoe }}<br />T.
                      {{ item.fid_tramitador.telefonos }}
                    </td>
                    <td>
                          ...
                          <template #verDetalle>
                            <router-link
                              ...
                              :to="{
                                ...
                                params: {
                                  id_solicitud_vincular_tramitador:
                                    item.id_solicitud_vincular_tramitador,
                                },
                              }"
                            >
                              Ver Detalle
...
import AccionesLista from "@/modules/senasag/components/solicitudes/vincular-tramitador/ActionsList.vue";

export default {
  name: "ListaSolicitudesVincularTramitador",
  order: 5,
  ...
  data() {
    return {
      title: "Listado de Solicitudes vincular Tramitadores",
      items: [
        {
          text: "Solicitud",
          href: "/",
        ...
        {
          text: "Lista Solicitud Registro Tramitadores",
          active: true,
        },
      ],
      columns: [
        { label: "Fecha", order_key: "fecha_solicitud" },
        { label: "Empresa", order_key: "fid_persona__nombres" },
        { label: "Nombre y Apellido", order_key: "fid_persona__nombres" },
  ...
  methods: {
    ...
    fullName(data) {
      return `${data.fid_tramitador.nombres} ${
        data.fid_tramitador.apellido_paterno
      } ${data.fid_tramitador.apellido_materno ?? ""}`;
    },
    companyName(data) {
      return data.fid_empresa === undefined || data.fid_empresa === null
        ? ""
        : data.fid_empresa.razonsocial;
    },
